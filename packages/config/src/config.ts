...
export const RuntimeConfigSchema = Type.Object(
  {
    ...
    authCacheTtlSeconds: Type.Integer({ minimum: 0, maximum: 60 }),
...
const fieldToEnvironmentVariable: Readonly<Record<string, string>> = {
  ...
  authCacheTtlSeconds: 'AUTH_CACHE_TTL_SECONDS',
...
export function loadConfig(source: EnvironmentSource): RuntimeConfig {
  const candidate = {
    ...
    authCacheTtlSeconds: parseInteger(source.AUTH_CACHE_TTL_SECONDS, 30),
