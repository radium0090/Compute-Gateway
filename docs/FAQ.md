...
## What is RAX Compute Gateway?

RAX Compute Gateway is a self-hostable gateway that exposes one OpenAI-compatible API for
multiple AI model providers and centralizes routing, credentials, reliability,
limits, and telemetry.

## Is RAX Compute Gateway a reverse proxy?
...
Yes, for the supported Chat Completions subset. Change the base URL to RAX Compute Gateway
and use a RAX Compute Gateway API key. Unsupported OpenAI fields return explicit errors.

## Which providers are supported first?

OpenAI, Anthropic, and Gemini. Models are operator-configured because provider
catalogs and capabilities change independently of RAX Compute Gateway releases.

## Does `rax/fast` always use the same model?
...
No. RAX Compute Gateway may fall back before response commitment. After streaming begins, a
failure ends that stream; output from a second model is never spliced in.

## Does RAX Compute Gateway store prompts or completions?
...
RAX Compute Gateway keys are high-entropy credentials stored as a keyed one-way hash. The
...
## Does RAX Compute Gateway guarantee lower cost or better answers?
...
No. The open-source deployment works without a RAX Compute Gateway account. A future hosted
